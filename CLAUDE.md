...
- ADO build (run) JSON omits `result`/`finishTime` while in progress, so under
  `Set-StrictMode -Version Latest` accessing them throws "property cannot be found".
  Use a `PSObject.Properties[name]` safe accessor. Timeline records, by contrast,
  include those properties as `null`.

## Skills

### azure-pipeline-monitor

`skills/azure-pipeline-monitor/` monitors an Azure DevOps pipeline run — the whole run
or a single stage/job — to completion. The work is a blocking poller,
`scripts/Watch-AdoPipeline.ps1`, that holds control until a final outcome and prints a
JSON result on stdout (progress to stderr).

- **Auth:** `-Pat` / `AZURE_DEVOPS_PAT` (Basic) else an `az account get-access-token`
  Bearer token for resource `499b84ac-1321-427f-aa17-267ca6975798`, auto-refreshed for
  multi-hour runs.
- **Targets:** `-BuildUrl` (parses org/project/buildId from a `dev.azure.com` or
  `*.visualstudio.com` URL) or explicit `-Organization`/`-Project`/`-BuildId`. Add
  `-RecordName` (+ optional `-RecordType Stage|Phase|Job|Task`) for a single timeline
  record.
- **Adaptive cadence:** estimates median duration from up to `-HistorySamples` past
  *completed* runs (regardless of result — unofficial defs rarely fully `succeed`),
  then polls every `clamp(remainingToExpectedFinish / 5, MinInterval=15s,
  MaxInterval)`. `MaxInterval` auto-scales with the estimate (`expected/10`, clamped
  120s–600s) so long runs aren't polled excessively; a fixed `-MaxIntervalSeconds`
  overrides it. Robust to bad estimates; waits at a 120s ceiling with no history.
- **Run it backgrounded/detached** with a long wait — it blocks for the life of the
  pipeline. Treat `succeededWithIssues`/`partiallySucceeded` as success, and `timedOut`
  as "watcher gave up", not a pipeline failure.

REST API endpoints used: `GET _apis/build/builds/{id}`,
`GET _apis/build/builds?definitions={defId}&statusFilter=completed`, and
`GET _apis/build/builds/{id}/timeline`.
